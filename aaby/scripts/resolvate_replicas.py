...
def resolvate_only(base="system", mdp="mdps/step6.0_minimization.mdp", water='OPC', ions="Na+,Cl-", conc="0.15"):
    ...
    fix_topol()
    fix_itp(base)
    run(' '.join([sys.executable, "-m", "aaby.scripts.generate_restraints"]))
...
if __name__ == "__main__":
    ...
    if args.membrane:
        default_mdp_path = MDPS_DIR / "MEMBRANE" / "step6.0_minimization.mdp"
        min_mdp_path     = MDPS_DIR / "MEMBRANE" / "step5_mini.mdp"
    else:
        default_mdp_path = MDPS_DIR / "SOLUTION" / "step6.0_minimization.mdp"
        min_mdp_path     = MDPS_DIR / "SOLUTION" / "step5_mini.mdp"

    default_mdp = str(default_mdp_path)
